...
import { makeMapCacheKey } from 'maprender/mapCodec'
import type {
...
import { ActorManager } from 'maprender/actorManager'
import { EntityStore, type EntitySnapshot } from 'maprender/entityStore'
import { createMapTileMap, createServerColliderDebugActor } from 'maprender/mapTileMap'
...
export class MyGame extends Engine {
   ...
   * 职责：
   * - 记录本地玩家 entityId
   * - 把 per-client 可见实体表（PlayerState.visibleEntities）写入 EntityStore
   * - 记录昼夜（hour/phase）与本地玩家地图（PlayerState.mapId）；mapId 变化时重建地图层
   *
   * @param state 服务端房间状态
  ...
  private handleRoomStateChange(state: RoomState, sessionId: string) {
    ...
    const myMapId = state.players.get(sessionId)?.mapId ?? ''
    ...
    if (prevMapId !== myMapId && prevMapId !== '' && myMapId) {
      this.rebuildMapForWorld(myMapId)
    }
  }
