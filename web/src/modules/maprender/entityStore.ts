import type { EntityState, RoomState } from 'net/schema'
...
export class EntityStore {
   ...
   *
   * 实体来源（per-client 协议）：
   * - 恒走自己的 visibleEntities（兴趣裁剪 per-client 可见表）
   * - 空/不可解码时返回空快照表（等待服务端填充）
   *
   * @param state 房间状态
  ...
  private readEntities(state: RoomState, sessionId: string) {
    ...
    const visible = state.players.get(sessionId)?.visibleEntities
    if (!visible || visible.size === 0 || !EntityStore.isDecodable(visible)) return map
